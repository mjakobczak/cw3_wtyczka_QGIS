...
class Wtyczka:
    ...
    def run(self):
		...
		jsonPath=os.path.join(os.path.expanduser("~"), "Downloads/weather.json")
		...
		layer=QgsVectorLayer("C:/Users/jakob_000/Desktop/WTYCZKA QGIS/powiaty_lodzkie.shp", "Weather", "ogr")
			...
			for region in layer.getFeatures():
					if region.geometry().contains(QgsGeometry.fromPoint(point)):
						for i in xrange(0, len(attributes)):
							region.setAttribute(attributes[i], attributesValues[i])
